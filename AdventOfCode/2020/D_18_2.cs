    ...
    public static class D_18_2
    {
        ...
        private static long SolveEquation(string input)
        {
            ...
            while (input.Contains("+"))
            {
                ...
                Match match = regex.Match(input);
                long total = CalculateEquation(match.Groups[0].Value);

                string firstBitOfInput = input.Substring(0, match.Index);
                string lastBitOfNewInput = input.Substring(match.Index + match.Length);

                input = $"{firstBitOfInput}{total.ToString()}{lastBitOfNewInput}";
            }
