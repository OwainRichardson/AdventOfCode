...
namespace AdventOfCode._2019
{
    public static class D_03_2
    {
        ...
        private static void GeneratePathCoordinates(string instructions, List<Coord> coords)
        {
            var splitInstructions = instructions.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);

            foreach (var instruction in splitInstructions)
            {
                var direction = instruction.Substring(0, 1);
                var distancePart = instruction.Substring(1);
                if (direction.Equals("R"))
                {
                    var distance = int.Parse(distancePart);

                    var currentCoord = coords.Last();
