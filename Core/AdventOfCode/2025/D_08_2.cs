...
public static class D_08_2
{
    public static void Execute()
    {
        ...
        while (true)
        {
            ...
            if (canFinish && circuits.Count == 1 && circuits.Single().Value.Count == junctionBoxes.Count)
            {
                Console.WriteLine(boxToConnect1.X * boxToConnect2.X);

                break;
            }

            if (!canFinish && circuits.Count > 1)
            {
                canFinish = true;
            }
