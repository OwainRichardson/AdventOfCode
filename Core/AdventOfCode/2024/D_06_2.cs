...
namespace AdventOfCode._2024
{
    public static class D_06_2
    {
        public static void Execute()
        {
            ...
            for (int obstructionY = 0; obstructionY < inputs.Length; obstructionY++)
            {
                for (int obstructionX = 0; obstructionX < inputs.Length; obstructionX++)
                {
                    (char[,] map, int x, int y) = ParseInputs(inputs);

                    Direction direction = Direction.Up;

                    if (map[obstructionX, obstructionY] != '^' && map[obstructionX, obstructionY] != '#')
                    {
                        map[obstructionX, obstructionY] = '#';
            ...
            Console.WriteLine(loopLocations);
        }
