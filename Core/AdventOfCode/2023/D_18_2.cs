    ...
    public static class D_18_2
    {
        public static void Execute()
        {
            ...
            while (coordCounted)
            {
                ...
                foreach (var coord in inners.TakeLast(5000))
                {
        ...
        private static List<DigInstruction> ParseInputToInstructions(string[] inputs)
        {
            ...
            foreach (string input in inputs)
            {
                ...
                string hexCode = match.Groups[3].Value;
                string prefixedHex = $"0x{hexCode.Substring(1, 5)}";
                int distance = Convert.ToInt32(prefixedHex, 16);

                DigInstruction instruction = new DigInstruction
                {
                    Direction = ParseDirection(hexCode.Last().ToString()),
                    Number = distance,
                    HexCode = hexCode
                };
